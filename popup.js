document.addEventListener("DOMContentLoaded", function () {
      ...
      function (data) {
        if (data.apiKey || data.access_token) {
          fetchMemories(data.userId, data.apiKey, data.access_token).then(
            () => {
              if (newMemory) {
                // Scroll to the bottom of the memories list only if newMemory is true
  ...
  function fetchMemories(userId, apiKey, accessToken) {
    ...
    return fetch(`https://api.mem0.ai/v1/memories/?user_id=${userId}`, {
      ...
      .catch((error) => {
        ...
        memoriesContainer.innerHTML = "<p>Error fetching memories</p>";
      });
  }
  ...
  function displayMemories(memories) {
    ...
    // Create and append addMemoryButton
    ...
    memoryWrapper.id = "memory-wrapper";
    memoryWrapper.className = "memory-wrapper";

    const allMemoriesLabel = document.createElement("div");
    allMemoriesLabel.textContent = "All Memories";
    allMemoriesLabel.style.cssText = `
      font-size: 12px;
      color: #666;
      margin-bottom: 5px;
      padding-left: 15px;
    ...
    if (memories.length === 0) {
      ...
      memoryWrapper.appendChild(allMemoriesLabel);
      memoryWrapper.appendChild(scrollArea);
    }
    memoriesContainer.appendChild(memoryWrapper);
    // Create shortcut info div
  ...
  function addNewMemory() {
    // Implement the logic to add a new memory
